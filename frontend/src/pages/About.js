    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6">Our Mission & Vision</h1>
      ...
      <section id="vision" className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {[
              { title: 'Our Vision', icon: <Target className="h-7 w-7 text-white" />, bg: 'from-blue-500 to-blue-600', text: 'To raise self-reliant and God fearing generations in South Sudan and beyond.', footerIcon: <Heart className="h-4 w-4 mr-2 text-blue-600" />, footerText: 'Inspiring future leaders' },
              { title: 'Our Mission', icon: <BookOpen className="h-7 w-7 text-white" />, bg: 'from-green-500 to-green-600', text: 'Heavenly Nature Ministry exists to empower street/abandoned children and orphans to fully utilize their spiritual, physical and mental gifts to become self-reliant and harmonious citizens in society.', footerIcon: <Users className="h-4 w-4 mr-2 text-green-600" />, footerText: 'Empowering through education' },
              { title: 'Our Goals', icon: <Target className="h-7 w-7 text-white" />, bg: 'from-purple-500 to-purple-600', list: ['Empowering children spiritually, physically and mentally','Teaching obedience and honor for Godʼs word','Building useful citizens through biblical principles'], footerIcon: <HandHeart className="h-4 w-4 mr-2 text-purple-600" />, footerText: 'Transformative community impact' }
            ].map((item, i) => (
              <Card key={i} className="border-0 hover:shadow-xl transition-transform duration-300 hover:-translate-y-1">
                <CardHeader>
                  <div className={`bg-gradient-to-r ${item.bg} rounded-full p-3 w-14 h-14 flex items-center justify-center mb-4`}>{item.icon}</div>
                ...
                <CardContent>
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-100 flex items-center text-sm text-gray-500">
                    {item.footerIcon}<span>{item.footerText}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {coreValues.map((value, idx) => (
              <Card key={idx} className="hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-0 shadow-md" aria-label={`${value.meaning}: ${value.description} (${value.scripture})`}>
      ...
      <section className="py-16 bg-gradient-to-r from-blue-600 to-indigo-700 text-white text-center">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button asChild size="lg" className="bg-white text-blue-600 hover:bg-gray-100"><a href="/contact">Partner With Us</a></Button>
            <Button asChild size="lg" variant="outline" className="border-white text-white hover:bg-white/10"><a href="/donate">Support Our Work</a></Button>
            <Button asChild size="lg" variant="outline" className="border-white text-white hover:bg-white/10"><a href="/events">Attend an Event</a></Button>
          </div>
        </div>
      </section>
