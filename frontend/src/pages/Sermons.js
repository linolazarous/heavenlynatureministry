    ...
    <div className="min-h-screen">
      <section className="bg-primary text-white py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl text-white/90">Listen to powerful messages that transform lives</p>
        </div>
      </section>
      ...
      <section className="section-padding bg-background">
        <div className="max-w-7xl mx-auto">
          {loading ? (
              ...
              {sermons.map((sermon) => (
                <Card key={sermon.id} className="card-hover" data-testid={`sermon-card-${sermon.id}`}>
                  <div className="relative aspect-video overflow-hidden rounded-t-xl">
                    <img
                      src={sermon.thumbnail_url || "https://images.unsplash.com/photo-1716666178997-6e4a056f07d1?crop=entropy&cs=srgb&fm=jpg&q=85"}
                      alt={sermon.title}
                  ...
                  <CardContent>
                    <p className="text-muted-foreground mb-4 line-clamp-2">{sermon.description}</p>
            ...
            <div className="text-center py-20">
              <p className="text-lg text-muted-foreground">No sermons available yet. Check back soon!</p>
            </div>
          )}
        </div>
      </section>
